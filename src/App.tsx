...
import { Activity, Pill, User, HeartPulse, Clock, CalendarHeart, Plus, Check, Github, Stethoscope, ChevronRight, AlertCircle, Thermometer, X, AlertTriangle, Sun, Moon } from 'lucide-react';
...
export default function App() {
  ...
  const handleSavePatient = () => {
    ...
    const newPatient: Patient = {
      ...
      doctorInCharge: newPatientData.doctorInCharge,
      medications: [],
      vitalsLogs: [],
      symptomLogs: [],
      medicationTakeLogs: []
      ...
      <aside className="w-full md:w-80 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-sm z-10 md:min-h-screen">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <button 
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-amber-400 transition-colors"
          >
        ...
        <div className="mt-auto p-6 border-t border-slate-100 dark:border-slate-800">
           <a 
            href="https://github.com/Death779/health-tracker-app/tree/main" 
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white rounded-lg text-sm font-medium transition-colors"
          >
            <Github size={16} />
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-4 flex-1">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Лечащий врач</label>
                <select
                  value={newPatientData.doctorInCharge}
                  onChange={e => setNewPatientData({...newPatientData, doctorInCharge: e.target.value})}
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                >
                  {DOCTORS.map(d => (
